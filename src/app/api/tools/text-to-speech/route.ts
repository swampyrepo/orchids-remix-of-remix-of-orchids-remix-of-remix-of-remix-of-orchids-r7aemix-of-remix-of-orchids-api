...
export async function GET(req: NextRequest) {
  ...
  const speakingId = searchParams.get("speaking-id");
  ...
  for (const apiKey of apiKeys) {
    try {
      const response = await fetch(
        `https://api.elevenlabs.io/v1/text-to-speech/${speakingId}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        await logApiRequest({
          ip_address: userIP,
          method: "GET",
          router: "/api/tools/text-to-speech",
          status: 200,
          user_agent: realUA
        });
